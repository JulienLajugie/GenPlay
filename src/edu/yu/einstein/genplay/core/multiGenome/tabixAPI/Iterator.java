	...
	public String next() throws IOException {
		...
		for (;;) {
			...
			if ((s = tabixReader.readLine(reader)) != null) {
				...
				intv = getIntv(s);
				if (intv.getTid() != chromosomeIndex || intv.getBeg() >= stop) break; // no need to proceed
				else if (intv.getEnd() > start && intv.getBeg() < stop) return s; // overlap; return
	...
	private TIntv getIntv(final String s) {
		...
		while ((end = s.indexOf('\t', beg)) >= 0 || end == -1) {
			...
			if (col == tabixReader.getmSc()) {
				...
				if (intv.getBeg() < 0) intv.setBeg(0);
				if (intv.getEnd() < 1) intv.setEnd(1);
