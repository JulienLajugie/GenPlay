...
public class Launcher {
	...
	public static void main(final String[] args) {
		...
		if (isDemo) {
			...
			try {
				ProjectRecordingManager.getInstance().createChromosomeManager(is);
			...
			try {
				f = new File(args[0]);
				ProjectRecordingManager.getInstance().createChromosomeManager(new File(args[0]));
				isProjectLoaded = true;
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private static void projectScreen() {
		...
		if (screenProject.isLoadingEvent()) {
			try {
				...
				ProjectRecordingManager.getInstance().createChromosomeManager();
			} catch (Exception e) {
				e.printStackTrace();
			}
		} else {
			...
			chromosomeManager.setProjectName(screenProject.getName());
			chromosomeManager.setCladeName(clade.getName());
			chromosomeManager.setGenomeName(genome.getName());
			chromosomeManager.setVarFiles(screenProject.getVarFiles());
			chromosomeManager.setAssembly(assembly);
		}
