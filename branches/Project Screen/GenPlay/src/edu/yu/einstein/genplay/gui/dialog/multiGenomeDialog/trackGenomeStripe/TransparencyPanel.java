...
class TransparencyPanel extends JPanel implements ChangeListener {
	...
	protected TransparencyPanel (int width) {
		//Dimension
		Dimension panelDim = new Dimension(width, MultiGenomePanel.getTransparencyPanelHeight());
		setSize(panelDim);
		...
		Dimension labelDim = new Dimension(MultiGenomePanel.getTransparencyLabelWidth(), MultiGenomePanel.getTransparencyLineHeight());
		...
		Dimension sliderDim = new Dimension(getSliderWidth(width), MultiGenomePanel.getTransparencyLineHeight());
		...
		Dimension valueDim = new Dimension(40, MultiGenomePanel.getTransparencyLineHeight());
		...
		gbc.gridx = 1;
		gbc.gridy = 0;
		gbc.insets = new Insets(MultiGenomePanel.getTransparencyInset(), getSliderLeftInset(), 0, 0);
		add(value, gbc);
	...
	private int getSliderWidth (int width) {
		int sliderWidth = width - (MultiGenomePanel.getTransparencyLabelWidth() + MultiGenomePanel.getHorizontalInset() + SLIDER_RIGHT_OFFSET);
		return sliderWidth;
	}
