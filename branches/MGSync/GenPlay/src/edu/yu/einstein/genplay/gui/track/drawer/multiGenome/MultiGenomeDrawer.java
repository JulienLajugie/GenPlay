...
/**
 * The multi genome drawer is in charge of drawing variation stripes for multi genome project. A track can display stripes of one or two alleles. If the display for both allele is required, the track
 * is then horizontally split. The list makers are in charge to create the list of variant. Drawing a stripe consist on: - drawing the stripe from the specific height and width, genome positions must
 * be translated to the screen positions - drawing the letters (nucleotide) for each stripe when they are known - changing the stripe display when the mouse goes over a stripe - displaying an
 * information dialog about a variant when the user clicks on it.
 * 
...
public class MultiGenomeDrawer implements Serializable {
	...
	public void resetVariantListMaker() {
		if (ProjectManager.getInstance().isMultiGenomeProject()) {
			...
			if (hasBeenReset) {
				...
				if (!showReference) {
					updateReferenceDisplayPolicy();
					// allele01VariantListMaker.fitToScreen();
					// allele02VariantListMaker.fitToScreen();
				}
			}
		}
	}
	...
	/**
	 * Updates information for multi genome project. These information are about: - stripes - filters
	...
	private void updateReferenceDisplayPolicy() {
		List<VariantDisplay> list01 = allele01VariantListMaker.getVariantList();
		List<VariantDisplay> list02 = allele02VariantListMaker.getVariantList();
		ByteArrayAsIntegerList display01 = allele01VariantListMaker.getDisplayPolicy().getDisplayPolicyList();
		ByteArrayAsIntegerList display02 = allele02VariantListMaker.getDisplayPolicy().getDisplayPolicyList();
		MultiGenomeProject multiGenomeProject = ProjectManager.getInstance().getMultiGenomeProject();

		for (int i = 0; i < list01.size(); i++) {
			if (list01.get(i).isReference() && list02.get(i).isReference()) {
				int positionIndex = multiGenomeProject.getReferencePositionIndex(chromosome, list01.get(i).getSource().getReferenceGenomePosition());
				display01.set(positionIndex, VariantDisplayPolicy.DO_NOT_DISPLAY);
				display02.set(positionIndex, VariantDisplayPolicy.DO_NOT_DISPLAY);
			}
		}
		...
		if ((variantDataList != null) && (variantDataList.size() > 0)) {
			...
			if (!locked) { // if there are stripes
				...
				if (trackAlleleType == AlleleType.BOTH) { // if both allele must be displayed
					...
					variantDrawer.drawGenome(allele01Graphic, genomeWindow, allele01VariantListMaker.getDisplayPolicy(), allele01VariantListMaker.getFittedData(genomeWindow, xFactor)); // draw the
					// stripes for
					// the first
					// allele
					variantDrawer.setCurrentAllele(AlleleType.ALLELE02);
					variantDrawer.drawGenome(allele02Graphic, genomeWindow, allele02VariantListMaker.getDisplayPolicy(), allele02VariantListMaker.getFittedData(genomeWindow, xFactor)); // draw the
					// stripes for
					// the second
					// allele
	...
	public void toolTipStripe(int trackHeight, MouseEvent e) {
		if (ProjectManager.getInstance().isMultiGenomeProject()) { // we must be in a multi genome project
			...
			if (variant != null) { // if a variant has been found
				...
				List<VariantDisplay> variantList = null; // we will try to get the full list of variant displayed on the whole track (we want to move from a variant to another one no matter the
				// allele)
	...
	public boolean isOverVariant(int trackHeight, MouseEvent e) {
		if (ProjectManager.getInstance().isMultiGenomeProject() && !locked && (allele01VariantListMaker != null) && (allele02VariantListMaker != null)) { // if we are in multi genome project
			double pos = projectWindow.screenXPosToGenomePos(TrackGraphics.getTrackGraphicsWidth(), e.getX()); // we translate the position on the screen into a position on the genome
			VariantDisplay variant = getDisplayableVariant(trackHeight, pos, e.getY()); // we get the variant (Y is needed to know if the variant is on the upper or lower half of the track)
			if (variant != null) { // if a variant has been found
	...
	/**
	 * @param variant
	 * @return true if the variant is shown, false otherwise
	 */
	public boolean isVariantShown(VariantDisplay variant) {
		boolean allele01 = allele01VariantListMaker.getDisplayPolicy().isShown(variant);
		boolean allele02 = allele02VariantListMaker.getDisplayPolicy().isShown(variant);
		if (allele01 || allele02) {
			return true;
		}
		return false;
