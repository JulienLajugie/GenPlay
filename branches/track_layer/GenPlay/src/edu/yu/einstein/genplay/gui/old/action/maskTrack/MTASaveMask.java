 ...
 *******************************************************************************/
package edu.yu.einstein.genplay.gui.old.action.maskTrack;
 ...
 * @version 0.1
 */
public final class MTASaveMask extends TrackListActionWorker<Void> {

	private static final long serialVersionUID = 212223991804272305L;	// generated ID
	private static final String 	ACTION_NAME = "Save Mask";			// action name
	private static final String 	DESCRIPTION =
			"Save the mask of the selected track";								 		// tooltip
	private Writer 					writer;								// object that writes the data

	/**
	 * key of the action in the {@link ActionMap}
	 */
	public static final String ACTION_KEY = "ATASaveMask";


	/**
	 * Creates an instance of {@link MTASaveMask}
	 */
	public MTASaveMask() {
		super();
	...
	protected Void processAction() throws Exception {
		if (getTrackList().getSelectedTrack() != null) {
			...
			jfc.setDialogTitle("Save Mask Track");
			Track<?> selectedTrack = getTrackList().getSelectedTrack();
			FileFilter[] filters = Utils.getWritableSCWFileFilter();
			...
			if(returnVal == JFileChooser.APPROVE_OPTION) {
				...
				if (!Utils.cancelBecauseFileExist(getRootPane(), selectedFile)) {
					ChromosomeListOfLists<?> data = selectedTrack.getMask();
					String name = selectedTrack.getName();
					writer = WriterFactory.getWriter(selectedFile, data, name, selectedFilter);
					notifyActionStart("Saving Mask Track #" + selectedTrack.getTrackNumber(), 1, writer instanceof Stoppable);
					writer.write();
				}
			}
		}
