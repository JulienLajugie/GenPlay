...
/**
 * Sets a specified value to the scores of each bin of a {@link BinList}
 * @author Julien Lajugie
 * @version 0.1
 */
public class BLOUniqueScore implements Operation<BinList> {
	...
	/**
	 * Adds a specified value to the scores of each bin of a {@link BinList}
	 * @param binList input {@link BinList}
	 * @param constant constant to add
	 */
	public BLOUniqueScore(BinList binList, double constant) {
				...
				public List<Double> call() throws Exception {
					...
					if ((currentList != null) && (currentList.size() != 0)) {
						...
						for (int j = 0; (j < currentList.size()) && !stopped; j++) {
							if (currentList.get(j) == 0.0) {
								resultList.set(j, 0.0);
							} else {
								resultList.set(j, constant);
							}
						}
					}
