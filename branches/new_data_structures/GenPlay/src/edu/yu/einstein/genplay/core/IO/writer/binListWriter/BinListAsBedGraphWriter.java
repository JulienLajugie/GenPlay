...
public final class BinListAsBedGraphWriter extends BinListWriter implements Stoppable {
	 ...
	 */
	public BinListAsBedGraphWriter(File outputFile, BinList data, String name) {
					...
					for (int j = 0; j < currentList.size(); j++) {
						...
						if (currentList.get(j).getScore() != 0) {
							...
							if ((start > -1) && (stop > -1)) {
								...
								writer.write(currentChromosome.getName() + "\t" + (j * binSize) + "\t" + stop + "\t" + currentList.get(j).getScore());
								writer.newLine();
							}
						}
					}
