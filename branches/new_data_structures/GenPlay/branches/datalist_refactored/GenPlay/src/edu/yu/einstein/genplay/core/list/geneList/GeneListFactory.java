...
import edu.yu.einstein.genplay.core.reader.GeneReader;
...
public class GeneListFactory {
	...
	public static GeneList createGeneList(final GenomicDataList<String> nameList, final GenomicDataList<Strand> strandList,
			final GenomicDataList<Integer> startList, final GenomicDataList<Integer> stopList, final GenomicDataList<Integer> UTR5BoundList,
			final GenomicDataList<Integer> UTR3BoundList,final GenomicDataList<int[]> exonStartsList,
			final GenomicDataList<int[]> exonStopsList, final GenomicDataList<double[]> exonScoresList, String geneDBURL)
				...
				public List<Gene> call() throws Exception {
					...
					for(int j = 0; j < nameList.size(currentChromosome); j++) {
						...
						if (txStop < currentChromosome.getLength()) {
							resultList.add(new Gene(name, currentChromosome, strand, txStart, txStop, UTR5Bound, UTR3Bound, exonStarts, exonStops, exonScores));
						}
					}
		...
		return createGeneList(result, geneDBURL);
	}
	 ...
	 * @throws InterruptedException
	 */
	public static GeneList createGeneList(List<? extends List<Gene>> data) throws InterruptedException, ExecutionException {
		return createGeneList(data, null);
	}
	 ...
	 * @throws InterruptedException
	 */
	public static GeneList createGeneList(List<? extends List<Gene>> data, String geneDBURL) throws InterruptedException, ExecutionException {
		GeneList geneList = new GeneList();
			...
			Callable<List<Gene>> currentThread = new Callable<List<Gene>>() {
				...
				public List<Gene> call() throws Exception {
					...
					for (int j = 0; j < currentList.size(); j++) {
						...
						Gene geneToAdd = new Gene(name, chromosome, strand, start, stop, exonStarts, exonStops, exonScores);
